...
| `NEXT_PUBLIC_AMAZON_TAG` | `sososi27-22` | AmazonアソシエイトのトラッキングID。**空文字を渡すとアフィリエイトリンクも開示文も一切出ない**（リンクが無いのに「収入を得ています」と書くと事実に反するため）。URLに載って公開される情報なので秘密ではない。Creators API の認証情報とは別物で、そちらは絶対にリポジトリに入れないこと |
