...
1. **他社のfps数値表をDB化しない。** レビューサイト、YouTube等の計測値を収集して
   自前のデータベースにするのは禁止。データベースの著作物にあたり、利用規約違反にもなる。
   **係数だけは例外**（2026-08-30 変更・2026-09-09 と 2026-09-17 に1項目ずつ追加・詳細は CONTEXT.md）。
   同一構成で条件だけを変えた第三者の測定から、解像度係数・設定係数・
   ゲーム間の重さ・1% Low ÷ 平均fps・GPU性能の効き方（性能指数に対するfpsの伸び方）・
   場面の重さ（同じ構成で場所だけを変えた測定の比）という**割り算の結果のみ**取ってよい。GPUだけを替えた測定も「条件だけを変えた」に含む。
   出典と算出過程を CONTEXT.md に必ず残す。
   fps数値そのものは保存しない。モデル別の値はメーカー公式スペックからの
   自前計算のまま。他に使ってよいのは自前の実測とユーザー投稿。
...
5. **感度換算の係数（yaw）は別枠として扱う。** これはfpsの測定値ではなく
   ゲーム仕様の定数なので、許諾を得た第三者サイトの公開値を使ってよい（2026-09-22 追加）。
   条件は3つ: 出典を CONTEXT.md に残す／**複数ソースの一致か自前の実測で確かめる**／
   サイトが公開している換算の関係をビルド時の自己テストで再現する。
   根拠が取れないタイトルは載せない（Rainbow Six Siege は対象外）。
...
- `ROADMAP.md` — 作るものと順番。全5フェーズの実装単位、完成の判定基準、
  依存関係、難所、未決事項。**新しい機能に着手する前に読む。**
